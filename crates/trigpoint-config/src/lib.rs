...
//!
//! This crate is deliberately free of rustc types so the configuration
//! contract can be understood (and unit-tested) without a nightly compiler
//! in hand. It is stable-toolchain code shared by every trigpoint consumer
//! of `triglint.toml`: the nightly dylint library in `triglint/`
//! path-depends on it, and so will the Python analysis.
...
mod tests {
    ...
    #[test]
    fn unknown_top_level_sections_are_still_rejected() {
